...
void ParticleSystem::SpawnParticle()
{
	...
	if (mInfo.maxSpawnAngle > 0.0f)
	{
		...
		if (abs(Math::Dot(mInfo.spawnDirection, Vector3::YAxis)) > 0.99f)
		{
	...
	data.colorStart = Lerp(mInfo.minStartColor, mInfo.maxStartColor, t);

	t = static_cast<float>(rand()) / static_cast<float>(RAND_MAX);
	data.colorEnd = Lerp(mInfo.minEndColor, mInfo.maxEndColor, t);

	t = static_cast<float>(rand()) / static_cast<float>(RAND_MAX);
	data.scaleStart = Lerp(mInfo.minStartScale, mInfo.maxStartScale, t);
