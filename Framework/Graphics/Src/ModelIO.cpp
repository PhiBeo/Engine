...
{
	uint32_t count = animation.mPositionsKeys.size();
	fprintf_s(file, "PositionKeyCount: %d\n", count);
	for (auto& k : animation.mPositionsKeys)
	{
...
void AnimationIO::Read(FILE* file, Animation& animation)
{
	...
	for (uint32_t k = 0; k < count; ++k)
	{
		...
		builder.AddRotaionKey(rot, time);
	}
...
void ModelIO::LoadModel(std::filesystem::path filePath, Model& model)
{
	filePath.replace_extension("model");
	FILE* file = nullptr;
	fopen_s(&file, filePath.u8string().c_str(), "r"); if (file == nullptr)
	{
		return;
	}
	uint32_t meshCount = 0;
	fscanf_s(file, "MeshCount: %d\n", &meshCount); model.meshData.resize(meshCount);
	for (uint32_t i = 0; i < meshCount; ++i)
	{
...
bool SpringEngine::Graphics::ModelIO::SaveMaterial(std::filesystem::path filePath, const Model& model)
{
...
void SpringEngine::Graphics::ModelIO::LoadMaterial(std::filesystem::path filePath, Model& model)
{
	filePath.replace_extension("material");
	FILE* file = nullptr;
